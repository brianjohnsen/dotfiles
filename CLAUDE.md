...
- IPv6 is disabled on the phone hotspot (`nmcli con mod johnsen ipv6.method disabled`). With it on, Chrome reaches Entra and Exchange/Teams over different address families (TDC mobile gives working IPv6), UFST's Conditional Access rejects the token as coming from another IP, and the Teams/Outlook PWAs loop between the password prompt and a 401. Citrix is unaffected. The setting lives in the NetworkManager profile, not in this repo — redo it after a fresh install.
